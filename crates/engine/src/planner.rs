...
use arrow_schema::{DataType, Field, Schema}; // DataType already imported below for evaluate_expression, but needed here too. Removed SchemaRef
...
impl PhysicalPlanner {
    ...
    pub fn create_physical_plan(
        ...
        match logical_plan {
            LogicalPlan::TableScan { table_name: _ } => {
                // table_name not used for dummy schema
...
pub fn evaluate_expression(batch: &RecordBatch, expr: &Expression) -> Result<ArrayRef, ArrowError> {
    match expr {
        Expression::BinaryExpr { left, op, right } => {
            ...
            let left_array = match &**left {
                ...
                _ => {
                    return Err(ArrowError::NotYetImplemented(
                        "Only column on left side supported".to_string(),
                    ))
                }
            };
            ...
            let right_val_str = match &**right {
                ...
                _ => {
                    return Err(ArrowError::NotYetImplemented(
                        "Only literal on right side supported".to_string(),
                    ))
                }
            };
            ...
            let int_array = left_array
                ...
                .ok_or_else(|| {
                    ArrowError::NotYetImplemented("Only Int32 column supported for GT".to_string())
                })?;

            let literal_val = right_val_str.parse::<i32>().map_err(|e| {
                ArrowError::InvalidArgumentError(format!(
                    "Could not parse literal '{}' as i32: {}",
                    right_val_str, e
                ))
            })?;
