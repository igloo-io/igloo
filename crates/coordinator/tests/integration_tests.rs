...
const SHUTDOWN_TIMEOUT_SECS: u64 = 15; // Define shutdown timeout constant
const MAX_GRPC_POLL_FAILURES: usize = 3; // Max consecutive gRPC poll failures
...
async fn test_milestone1_query_submission_and_planning() -> Result<(), anyhow::Error> {
    ...
    let server_handle = tokio::spawn(async move {
        ...
        if let Err(e) =
            run_server(addr, server_cluster_state, server_query_state, shutdown_rx).await
        {
    ...
    let mut client: CoordinatorServiceClient<Channel> =
        match CoordinatorServiceClient::connect(format!("http://{}", TEST_SERVER_ADDR)).await {
            Ok(c) => c,
            Err(e) => {
                ...
                return Err(anyhow::anyhow!(
                    "Failed to connect to test server: {}. Server was shut down.",
                    e
                ));
            }
        };
    ...
    let mut grpc_consecutive_failures = 0;
    ...
    while time_elapsed < polling_timeout {
        ...
        println!(
            "Polling attempt for {}: elapsed {:?}",
            query_id, time_elapsed
        );
        match client
        ...
        {
            Ok(response) => {
                grpc_consecutive_failures = 0; // Reset on success
            ...
            Err(e) => {
                grpc_consecutive_failures += 1;
                eprintln!(
                    "Error getting status for query {}: {:?}. Consecutive failures: {}/{}",
                    query_id, e, grpc_consecutive_failures, MAX_GRPC_POLL_FAILURES
                );
                if grpc_consecutive_failures >= MAX_GRPC_POLL_FAILURES {
                    anyhow::bail!(
                        "Max gRPC poll failures ({}) reached for query {}.",
                        MAX_GRPC_POLL_FAILURES, query_id
                    );
                }
            }
        }
    ...
    assert!(
        final_status_response.is_some(),
        "Polling timed out or query did not progress past 'Planned' state for query {}",
        query_id
    );

    let status_response = final_status_response.unwrap();
    println!(
        "Final polled status for query {}: {}",
        query_id, status_response.status
    );
    ...
    assert!(
        status_response.physical_plan_bytes.is_some(),
        "Physical plan bytes should not be None. Status was: {}",
        status_response.status
    );
    let plan_bytes = status_response.physical_plan_bytes.as_ref().unwrap();
    assert!(
        !plan_bytes.is_empty(),
        "Physical plan bytes should not be empty. Status was: {}",
        status_response.status
    );
    ...
    assert!(
        !status_response.tasks.is_empty(),
        "Should have generated some tasks. Status was: {}",
        status_response.status
    );
    println!(
        "Query {} check complete. Final status '{}', plan and tasks are populated.",
        query_id, status_response.status
    );
...
async fn test_milestone3_full_end_to_end_execution() -> Result<(), anyhow::Error> {
    ...
    let coordinator_handle = tokio::spawn(async move {
        ...
        if let Err(e) = igloo_coordinator::run_server(
            coordinator_addr,
            coord_cs_clone,
            coord_qs_clone,
            coord_shutdown_rx,
    ...
    let worker_handle = tokio::spawn(async move {
        println!(
            "M3 Worker {} starting on {}, connecting to {}...",
            worker_id_clone, worker_addr, coordinator_http_addr
        );
        if let Err(e) = igloo_worker::run_worker_server(
            worker_id_clone,
            worker_addr,
            coordinator_http_addr,
            worker_shutdown_rx,
        )
        .await
        {
    ...
    // 2. Test Logic
    println!(
        "M3: Connecting to coordinator server {}...",
        COORDINATOR_ADDR_M3
    );
    let mut client = CoordinatorServiceClient::connect(format!("http://{}", COORDINATOR_ADDR_M3))
        .await
        .map_err(|e| {
            // If connection fails, try to clean up spawned tasks.
            if let Some(tx) = opt_coord_shutdown_tx.take() {
                tx.send(()).ok();
            }
            if let Some(tx) = opt_worker_shutdown_tx.take() {
                tx.send(()).ok();
            }
    ...
    println!(
        "M3: Polling for 'Finished' status for query {}...",
        query_id
    );
    while time_elapsed < polling_timeout {
        tokio::time::sleep(poll_interval).await;
            ...
            .await
        {
            Ok(response) => {
                ...
                if status_resp.status.starts_with("Failed")
                    || status_resp.status.starts_with("DispatchFailed")
                {
            ...
            Err(e) => {
                grpc_consecutive_failures += 1;
                eprintln!(
                    "M3: Error getting status for query {}: {:?}. Consecutive failures: {}/{}",
                    query_id, e, grpc_consecutive_failures, MAX_GRPC_POLL_FAILURES
                );
                if grpc_consecutive_failures >= MAX_GRPC_POLL_FAILURES {
                    anyhow::bail!(
                        "M3: Max gRPC poll failures ({}) reached for query {}.",
                        MAX_GRPC_POLL_FAILURES, query_id
                    );
                }
            }
    ...
    assert!(
        final_query_status.is_some(),
        "M3: Polling timed out before query finished or failed for query {}",
        query_id
    );
    ...
    assert_eq!(
        status_response.status, "Finished",
        "Query status should be 'Finished'. Final status was: {}",
        status_response.status
    );
    assert!(
        status_response.finished_at.is_some(),
        "finished_at should be Some"
    );
    ...
    assert!(
        ...
        "Task statuses map should not be empty"
    );
    ...
    for (task_id, status) in task_statuses {
        assert_eq!(
            status, "Succeeded",
            "Task {} should have succeeded. Status was: {}",
            task_id, status
        );
    }
...
async fn test_milestone2_task_generation() -> Result<(), anyhow::Error> {
    ...
    let mut client: CoordinatorServiceClient<Channel> =
        match CoordinatorServiceClient::connect(format!("http://{}", TEST_SERVER_ADDR)).await {
            ...
            Err(e) => {
                ...
                server_handle.await?;
                return Err(anyhow::anyhow!(
                    "M2: Failed to connect to test server: {}. Server shut down.",
                    e
                ));
            }
        };
    ...
    while time_elapsed < polling_timeout {
            ...
            .await
        {
            Ok(response) => {
                ...
                let status_resp = response.into_inner();
                println!(
                    "M2: Status for {}: {}, Tasks count: {}",
                    query_id,
                    status_resp.status,
                    status_resp.tasks.len()
                );
            ...
            Err(e) => {
                grpc_consecutive_failures += 1;
                eprintln!(
                    "M2: Error getting status for query {}: {:?}. Consecutive failures: {}/{}",
                    query_id, e, grpc_consecutive_failures, MAX_GRPC_POLL_FAILURES
                );
                if grpc_consecutive_failures >= MAX_GRPC_POLL_FAILURES {
                    anyhow::bail!(
                        "M2: Max gRPC poll failures ({}) reached for query {}.",
                        MAX_GRPC_POLL_FAILURES, query_id
                    );
                }
            }
    ...
    assert!(
        final_status_response.is_some(),
        "M2: Polling timed out or tasks not generated for query {}",
        query_id
    );

    let status_response = final_status_response.unwrap();
    println!(
        "M2: Final polled status for query {}: {}",
        query_id, status_response.status
    );

    // Assertions for Task Generation
    assert_eq!(
        status_response.tasks.len(),
        2,
        "Expected 2 tasks to be generated."
    );
    ...
    for i in 0..2 {
        let task = status_response
            .tasks
            .get(i)
            .expect(&format!("Task {} should be present", i));

        let expected_task_id_suffix = format!("_task_{}", i);
        assert!(
            task.task_id.starts_with(&query_id),
            "Task ID {} should start with query_id {}",
            task.task_id,
            query_id
        );
        assert!(
            task.task_id.ends_with(&expected_task_id_suffix),
            "Task ID {} should end with {}",
            task.task_id,
            expected_task_id_suffix
        );

        let deserialized_payload: String = bincode::deserialize(&task.payload).map_err(|e| {
            anyhow::anyhow!(
                "Failed to deserialize task payload for task {}: {}",
                task.task_id,
                e
            )
        })?;
        ...
        assert_eq!(
            deserialized_payload, expected_payload,
            "Payload for task {} did not match",
            task.task_id
        );
        println!(
            "M2: Task {} assertions passed. Payload: '{}'",
            task.task_id, deserialized_payload
        );
    }

    println!(
        "M2: All task generation assertions passed for query {}.",
        query_id
    );
