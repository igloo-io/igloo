...
pub fn plan_query(sql: &str) -> Result<Vec<u8>> {
    ...
    let physical_plan = match ast {
        Statement::Query(_) => EnginePhysicalPlan::Scan { // Assuming SELECTs might become Scans
...
pub async fn dispatch_parallel_task(sql: &str) -> Result<()> {
    ...
    let original_serialized_plan = plan_query(sql)
        .context(format!("Failed to plan query for parallel dispatch: {}", sql))?;

    let original_physical_plan: EnginePhysicalPlan = bincode::deserialize(&original_serialized_plan)
    ...
    if partitioned_plans.len() == 1 && !matches!(partitioned_plans[0], EnginePhysicalPlan::Scan{..}) {
    ...
    } else if partitioned_plans.len() == 1 && matches!(partitioned_plans[0], EnginePhysicalPlan::Scan{..}) && num_workers > 1 {
    ...
    if plans_to_dispatch.len() == 1 && num_workers > 0 { // Single plan (e.g. non-scan, or scan for 1 worker)
        ...
        let future = async move {
            ...
            let mut client = IglooWorkerClient::connect(worker_address.to_string()).await
    ...
    } else { // Multiple plans for multiple workers (Scan case)
...
mod tests {
    ...
    fn test_plan_query_simple_select() {
        ...
        match plan.unwrap() {
            EnginePhysicalPlan::Scan { table_name, .. } => assert_eq!(table_name, "placeholder_table"),
